using MathsEngine.Utils;

namespace MathsEngine.Modules.Statistics.Dispersion
{
    public class ContinuousTableCalculator : IStandardDeviation
    {
        private readonly List<string> _classIntervals;
        private readonly List<int> _frequencies;
        private List<double> _midpoints;
        private readonly int _totalFrequency;

        public double Mean { get; private set; }
        public double Variance { get; private set; }
        public double StandardDeviation { get; private set; }

        public ContinuousTableCalculator(List<string> classIntervals, List<int> frequencies)
        {
            if (classIntervals == null || frequencies == null)
                throw new NullInputException();
            if (classIntervals.Count != frequencies.Count)
                throw new ListsNotSameSizeException();
            if (classIntervals.Count == 0)
                throw new EmptyDataSetException();
            if (frequencies.Any(f => f < 0))
                throw new InvalidFrequencyException();

            _classIntervals = classIntervals;
            _frequencies = frequencies;
            _totalFrequency = _frequencies.Sum();
        }

        public void Run()
        {
            CalculateMidpoints();
            CalculateMean();
            CalculateVarianceAndStdDeviation();
        }

        private void CalculateMidpoints()
        {
            _midpoints = new List<double>();
            foreach (var interval in _classIntervals)
            {
                var bounds = interval.Split('-');
                if (bounds.Length != 2 || !double.TryParse(bounds[0], out double lower) || !double.TryParse(bounds[1], out double upper))
                    throw new InvalidClassIntervalFormatException();

                _midpoints.Add((lower + upper) / 2);
            }
        }

        private void CalculateMean()
        {
            if (_totalFrequency == 0)
            {
                Mean = 0;
                return;
            }
            double sumOfFx = 0;
            for (int i = 0; i < _midpoints.Count; i++)
            {
                sumOfFx += _midpoints[i] * _frequencies[i];
            }
            Mean = sumOfFx / _totalFrequency;
        }

        private void CalculateVarianceAndStdDeviation()
        {
            if (_totalFrequency == 0)
            {
                Variance = 0;
                StandardDeviation = 0;
                return;
            }
            double sumOfFxSquared = 0;
            for (int i = 0; i < _midpoints.Count; i++)
            {
                sumOfFxSquared += (Math.Pow(_midpoints[i], 2) * _frequencies[i]);
            }
            Variance = (sumOfFxSquared / _totalFrequency) - Math.Pow(Mean, 2);
            StandardDeviation = Math.Sqrt(Variance);
        }


    }
}